...
type HTTPParallelUsers struct {
	...
	// request.
	AssertResponse func(_ *httptest.ResponseRecorder, code int, serveErr error)
}
...
func (hpu HTTPParallelUsers) serve(rf func() *http.Request, h httpserver.Handler) {
	...
	var user = func(wg *sync.WaitGroup, userID int) {
		for i := 1; i <= hpu.Loops; i++ {
			...
			code, serveErr := h.ServeHTTP(w, rf())
			if hpu.AssertResponse != nil {
				hpu.AssertResponse(w, code, serveErr)
			}
			// }(sl)
			time.Sleep(sl)
		}
